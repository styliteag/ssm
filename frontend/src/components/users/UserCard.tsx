...
export const UserCard: React.FC<UserCardProps> = ({
 ...
 const statusColor = user.enabled ? 'bg-success' : 'bg-destructive';
 ...
 <div>
 <CardTitle className="text-base font-w590">{user.username}</CardTitle>
 ...
 <div
 className="flex flex-col items-center justify-center p-3 rounded-md bg-white/[0.02] hover:bg-white/[0.04] transition-colors cursor-pointer border border-border"
 onClick={() => onViewKeys(user)}
 >
 <Key size={16} className="mb-1 text-primary" />
 <span className="text-xl font-w590">{user.keyCount || 0}</span>
 ...
 >
 <Shield size={16} className="mb-1 text-success" />
 <span className="text-xl font-w590">{user.authorizationCount || 0}</span>
