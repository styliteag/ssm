...
const HostsPage: React.FC = () => {
 ...
 <h4 className="font-w590 text-white flex items-center gap-2">
 ...
 {host.comment && (
 <p className="text-foreground text-xs mt-1" style={{ wordBreak: 'break-word' }}>
 <span className="font-w510">Comment:</span> {host.comment}
 </p>
 )}
 </div>
 ...
 {host.connection_status === 'online' && <CheckCircle size={14} className="text-success" />}
 {host.connection_status === 'offline' && <AlertCircle size={14} className="text-destructive" />}
 {host.connection_status === 'error' && <AlertCircle size={14} className="text-warning" />}
 {(!host.connection_status || host.connection_status === 'unknown') && <Activity size={14} className="text-muted-foreground" />}

 <span className="font-w510">
 ...
 <AlertCircle size={12} className="flex-shrink-0 text-destructive" />
 <span
 className="flex-1"
 style={{
 ...
 <div className="flex items-center gap-2 mb-1">
 <Key size={14} />
 <span className="font-w510">SSH Details</span>
 </div>
 <div className="text-xs text-foreground space-y-1" style={{ maxWidth: '100%' }}>
 <div style={{ wordBreak: 'break-word' }}>User: <code className="bg-surface-3 px-1 rounded">{host.username}</code></div>
 {host.key_fingerprint && (
 <div style={{ wordBreak: 'break-word' }}>
 Fingerprint: <code className="bg-surface-3 px-1 rounded text-xs" style={{ wordBreak: 'break-all' }}>
 ...
 <div className="flex items-center gap-2 mb-1">
 <Users size={14} />
 <span className="font-w510">Authorized Users ({host.authorizations.length})</span>
 </div>
 <div className="text-xs text-foreground space-y-1 max-h-20 overflow-y-auto">
 ...
 <code className="bg-surface-3 px-1 rounded flex-shrink-0" style={{ fontSize: '10px' }}>{auth.login}</code>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="flex items-center gap-2 mb-1">
 <Users size={14} />
 <span className="font-w510">Authorized Users</span>
 </div>
 <p className="text-muted-foreground text-xs">No authorized users</p>
 </div>
 )}
 </div>
 ), []);
 ...
 render: (value, host) => (
 <button
 className="font-w510 text-foreground hover:text-primary text-left cursor-pointer"
 onClick={(e) => {
 e.stopPropagation();
 ...
 render: (_, host) => {
 const icons = {
 online: <CheckCircle size={16} className="text-success" />,
 offline: <AlertCircle size={16} className="text-destructive" />,
 error: <AlertCircle size={16} className="text-warning" />,
 unknown: <Activity size={16} className="text-muted-foreground" />,
 disabled: <Ban size={16} className="text-muted-foreground" />
 };
 ...
 const colors = {
 online: 'text-success bg-success/10 dark:text-success dark:bg-success/20',
 offline: 'text-destructive bg-destructive/10 dark:text-destructive dark:bg-destructive/20',
 error: 'text-warning bg-warning/10 dark:text-warning dark:bg-warning/20',
 ...
 className="max-w-md"
 >
 <div className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-w510 cursor-pointer ${colors[currentStatus]}`}>
 {icons[currentStatus]}
 ...
 <Button onClick={() => setShowAddModal(true)} className=" shadow-primary/20">
